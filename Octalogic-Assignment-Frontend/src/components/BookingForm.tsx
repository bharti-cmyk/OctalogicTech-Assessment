...
export default function BookingForm() {
  ...
  const handleNext = () => {
    ...
    if (step === 2 && !formData.vehicleType)
      return toast.error("Select vehicle type");
    if (step === 3 && !formData.vehicleModel)
      return toast.error("Select model");
    if (step === 4 && (!formData.dateRange[0] || !formData.dateRange[1]))
  ...
  return (
    ...
    >
      <Paper
        elevation={3}
        sx={{ width: "100%", maxWidth: 800, p: 4, borderRadius: 3 }}
      >
              ...
              <Stack direction={{ xs: "column", md: "row" }} spacing={2}>
                <DatePicker
                  ...
                  onChange={(date) =>
                    setFormData({
                      ...formData,
                      dateRange: [date, formData.dateRange[1]],
                    })
                  }
                ...
                <DatePicker
                  ...
                  onChange={(date) =>
                    setFormData({
                      ...formData,
                      dateRange: [formData.dateRange[0], date],
                    })
                  }
                  slotProps={{ textField: { fullWidth: true } }}
                />
              ...
              <Typography>
                <strong>Vehicle Model:</strong>{" "}
                {
                  availableModels.find(
                    (m) => m.id.toString() === formData.vehicleModel,
                  )?.name
                }
              </Typography>
              <Typography>
                <strong>Date Range:</strong>{" "}
                {formData.dateRange[0]?.toLocaleDateString()} to{" "}
                {formData.dateRange[1]?.toLocaleDateString()}
              </Typography>
            </Stack>
          )}
        </Stack>
